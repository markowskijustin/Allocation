...
eq_transformed <- usmap_transform(data = sites.coded %>% filter(!is.na(lat)) %>% mutate(lon = as.numeric(long), lat = as.numeric(lat))) %>% 
  mutate(year = as.numeric(Year)) %>% 
  arrange(BHCMIS.ID, Site.Name, Year)


grafico <- plot_usmap() +
  geom_point(data = eq_transformed, aes(x = x, y = y, col = Site.State)) +
